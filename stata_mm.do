**********Data preparation**********
* open dataset
use "/Users/fenix/Dropbox/GSR/Continuous Bimatrix/Matching pennies/stata/mp_production_mean.dta", clear
...
gen subperiod = 0
replace subperiod = 1 if tick==0 & num_subperiods==25
replace subperiod = 2 if tick==1 & num_subperiods==25
replace subperiod = 3 if tick==2 & num_subperiods==25
replace subperiod = 4 if tick==3 & num_subperiods==25
replace subperiod = 5 if tick==4 & num_subperiods==25
replace subperiod = 6 if tick==5 & num_subperiods==25
replace subperiod = 7 if tick==6 & num_subperiods==25
replace subperiod = 8 if tick==7 & num_subperiods==25
replace subperiod = 9 if tick==8 & num_subperiods==25
replace subperiod = 10 if tick==9 & num_subperiods==25
replace subperiod = 11 if tick==10 & num_subperiods==25
replace subperiod = 12 if tick==11 & num_subperiods==25
replace subperiod = 13 if tick==12 & num_subperiods==25
replace subperiod = 14 if tick==13 & num_subperiods==25
replace subperiod = 15 if tick==14 & num_subperiods==25
replace subperiod = 16 if tick==15 & num_subperiods==25
replace subperiod = 17 if tick==16 & num_subperiods==25
replace subperiod = 18 if tick==17 & num_subperiods==25
replace subperiod = 19 if tick==18 & num_subperiods==25
replace subperiod = 20 if tick==19 & num_subperiods==25
replace subperiod = 21 if tick==20 & num_subperiods==25
replace subperiod = 22 if tick==21 & num_subperiods==25
replace subperiod = 23 if tick==22 & num_subperiods==25
replace subperiod = 24 if tick==23 & num_subperiods==25
replace subperiod = 25 if tick==24 & num_subperiods==25
...
* set block variable
gen block = 0
replace block = 1 if round<=10
replace block = 2 if round>10

* set second half varibale
gen second_half = 0
replace second_half = 1 if subperiod>=13

* drop first period of each priod and early data
drop if tick <=3 & num_subperiods == 25
drop if tick <=36 & num_subperiods == 0
drop if round==1 | round==5 | round==9 | round==13 | round==17
...
xtreg p1_diff p1_regret_sign p1_regret_sign_pure p1_regret_sign_8002 p1_regret_sign_IDDS ///
      p1_regret_sign_pure_8002 p1_regret_sign_pure_IDDS ///  
      if num_subperiods==25, fe vce(robust)
...
* open dataset
use "/Users/fenix/Dropbox/GSR/Continuous Bimatrix/Matching pennies/stata/mp_mm_cycles.dta", clear

* encode round_pair_id
encode session_round_id, gen (individual_id)

* create type dummy
gen type = 0
replace type = 1 if cyctype=="1.CW"
replace type = 2 if cyctype=="2.diagonal"
replace type = 3 if cyctype=="4.stay"
replace type = 4 if cyctype=="5.CCW"
...
* set second half varibale
gen second_half = 0
replace second_half = 1 if tick>=13 & num_subperiods==25
replace second_half = 1 if tick>=144 & num_subperiods==0

* create interaction term (8002, mm, pure, continuous)
gen dummy_8002 = 0
replace dummy_8002 = 1 if game == "8002"
gen dummy_mm = 0
replace dummy_mm = 1 if mean_matching == 1
...
xi:mlogit type dummy_continuous dummy_pure dummy_8002 dummy_continuous_pure dummy_continuous_8002 ///
		       ...
		       second_half i.block, cluster(individual_id) baseoutcome(1) 
outreg2 using C:\Users\fenix\Dropbox\stataresult, tex nonote se replace nolabel bdec(2)
